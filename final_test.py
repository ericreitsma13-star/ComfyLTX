#!/usr/bin/env python3
"""Single best-effort test: high-res SDXL ref → LTX I2V + audio."""
...
P = "young woman with long dark hair white dress, medium shot chest up facing viewer, walking on misty pine forest path at golden dawn, sharp, cinematic, photorealistic, detailed, singing, front view"
N = "headshot, close up, portrait, from behind, back view, ugly, deformed, blurry, low quality, cartoon"

# Step 1: Generate best SDXL reference at 1024x1024
print("Step 1: SDXL reference...")
sdxl_wf = {
    ...
    "23": {"class_type": "KSampler", "inputs": {"model": ["10",0], "positive": ["20",0], "negative": ["21",0], "latent_image": ["22",0], "seed": 600, "steps": 30, "cfg": 7.0, "sampler_name": "euler", "scheduler": "normal", "denoise": 1.0}},
        ...
        if h[pid]['status']['status_str'] == 'success':
            for no, out in h[pid].get("outputs",{}).items():
                for img in out.get("images",[]):
                    ...
                    print(f"  → {img['filename']} ({sz/1024:.0f} KB)")
                    import shutil
                    shutil.copy(fp, f"/home/ericr/ComfyUI/input/ref_best.png")
                    print("  → copied to input/ref_best.png")
        break
    time.sleep(2)

# Step 2: LTX I2V + audio
print("\nStep 2: LTX I2V + audio...")
vocals = "/home/ericr/ComfyUI/input/vocals_4s.wav"
if not os.path.exists(vocals):
    subprocess.run(["ffmpeg","-y","-i","/home/ericr/ComfyUI/input/pines_vocals.wav",
        "-t","4","-c","copy",vocals],check=True,capture_output=True)
...
ltx_wf = {
    ...
    "50": {"class_type": "RandomNoise", "inputs": {"noise_seed": 600}},
    ...
    "71": {"class_type": "SaveVideo", "inputs": {"video": ["70",0], "filename_prefix": "final_test", "format": "mp4", "codec": "h264"}},
}
