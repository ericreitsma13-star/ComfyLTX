#!/usr/bin/env python3
"""Test LTX 2.3 audio-conditioned I2V (SOTAI approach) with CondSafe LoRA."""
...
# Workflow using SOTAI's audio-conditioned approach
# Load model + audio VAE (from same checkpoint) + video VAE
wf = {
    ...
    # Encode audio into latent using audio VAE
    ...
    "53": {"class_type": "CFGGuider", "inputs": {
        "model": ["20", 0], "positive": ["32", 0], "negative": ["32", 1],
        "cfg": float(cfg)}},
    "54": {"class_type": "SamplerCustomAdvanced", "inputs": {
        "noise": ["50", 0], "guider": ["53", 0], "sampler": ["51", 0],
        "sigmas": ["52", 0], "latent_image": ["45", 0]}},

    # Separate AV latent into video + audio
    "60": {"class_type": "LTXVSeparateAVLatent", "inputs": {
        "av_latent": ["54", 0]}},

    # Decode video latent with tiled VAE decoder
    ...
    "71": {"class_type": "SaveVideo", "inputs": {
        "video": ["70", 0], "filename_prefix": "test_audio_cond",
        "format": "mp4", "codec": "h264"}},
}

print(f"Audio-conditioned I2V: {DURATION}s ({frame_count}frames) @ {W}x{H}")
print(f"CondSafe LoRA: 1.0, steps: {steps}, CFG: {cfg}, seed: {SEED}")
print(f"Audio: {AUDIO}, Image: {IMAGE}")
