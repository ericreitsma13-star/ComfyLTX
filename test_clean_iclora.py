#!/usr/bin/env python3
"""IC-LoRA with clean SDXL portrait + audio conditioning."""
...
PROMPT = "close-up portrait of a young woman with dark brown hair, singing softly, standing still in misty forest, cinematic lighting, subtle head movement"
...
wf = {
    ...
    "71": {"class_type": "SaveVideo", "inputs": {"video": ["70", 0], "filename_prefix": "iclora_clean", "format": "mp4", "codec": "h264"}},
}

print(f"IC-LoRA + clean reference: {DURATION}s ({frame_count}fr) @ {W}x{H}")
...
while time.time() - start < 600:
    ...
    if pid in h:
        ...
        print(f"Done in {elapsed:.0f}s — {data['status']['status_str']}")
        break
    time.sleep(2)
else: print("TIMEOUT"); sys.exit(1)

if raw_video and os.path.exists(raw_video):
    final = raw_video.replace(".mp4", "_final.mp4") if raw_video.endswith(".mp4") else os.path.join(OUTPUT_DIR, "iclora_clean_final.mp4")
    subprocess.run(["ffmpeg", "-y", "-i", raw_video, "-i", "/home/ericr/ComfyUI/input/pines_clip.mp3",
