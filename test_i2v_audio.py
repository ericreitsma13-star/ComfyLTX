#!/usr/bin/env python3
"""I2V + audio: char_clean reference, 832x480, low I2V strength for medium shot."""
...
PROMPT = "young woman with long dark hair, medium shot waist up facing viewer, walking on misty pine forest path at dawn, golden sunlight, cinematic, singing softly, forest visible behind her, front view"
NEG = "headshot, close up, extreme close up, portrait, from behind, back view, profile, looking away, ugly, deformed, blurry, low quality"
...
if not os.path.exists(vocals_8s):
    subprocess.run(["ffmpeg","-y","-i","/home/ericr/ComfyUI/input/pines_vocals.wav",
        "-t","8","-c","copy", vocals_8s], check=True, capture_output=True)

fc = max(9, ((int(round(duration*FPS))-1)//8)*8+1)
print(f"Res: {W}x{H}, {fc}frames ({fc/FPS:.1f}s), ref={REF_IMAGE}")
...
wf = {
    ...
    "71": {"class_type": "SaveVideo", "inputs": {"video": ["70",0], "filename_prefix": "test_i2v", "format": "mp4", "codec": "h264"}},
}
                ...
                for v in out.get("images", out.get("media",[])):
                    ...
                    if rv and os.path.exists(rv) and rv.endswith('.mp4'):
                        ...
                        r = subprocess.run(["ffprobe","-v","error","-show_entries",
                            "format=duration,size,bit_rate","-of","csv=p=0",final],
