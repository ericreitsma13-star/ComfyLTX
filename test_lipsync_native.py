#!/usr/bin/env python3
"""Native resolution lip sync test — no IC-LoRA, SOTAI approach."""
...
PROMPT = "close-up portrait of a young woman singing softly, standing still, misty pine forest at dawn, golden light"
NEGATIVE = "walking, running, ugly, deformed, bad anatomy, blurry, music"
W, H, FPS = 832, 480, 24
DURATION = 4.0
SEED = 42
steps, cfg = 20, 3.0
...
wf = {
    ...
    "71": {"class_type": "SaveVideo", "inputs": {
        "video": ["70", 0], "filename_prefix": "lipsync_native",
        "format": "mp4", "codec": "h264"}},
}

print(f"Native res lip sync: {DURATION}s ({frame_count}fr) @ {W}x{H}, seed={SEED}")
print(f"No IC-LoRA — testing raw audio conditioning")
...
if raw_video and os.path.exists(raw_video):
    final = os.path.join(OUTPUT_DIR, "lipsync_native_final.mp4")
