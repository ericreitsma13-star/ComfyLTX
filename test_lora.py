#!/usr/bin/env python3
"""Test with LTX LoRA + audio conditioning."""
...
PROMPT = "young woman with long dark hair, wearing a white dress, medium shot facing viewer, singing on a misty pine forest path at golden dawn, dappled sunlight through trees, cinematic, photorealistic, sharp focus, detailed, forest fog, front view, looking at camera"
NEG = "headshot, close up, extreme close up, portrait, from behind, back view, profile, ugly, deformed, blurry, low quality, cartoon, painting"
REF_IMAGE = "char_clean.png"
SEED = 55

vocals_8s = "/home/ericr/ComfyUI/input/vocals_8s.wav"
if not os.path.exists(vocals_8s):
    subprocess.run(["ffmpeg","-y","-i","/home/ericr/ComfyUI/input/pines_vocals.wav","-t","8","-c","copy",vocals_8s],check=True,capture_output=True)

fc = max(9, ((int(round(duration*FPS))-1)//8)*8+1)
print(f"LoRA test: {W}x{H} {fc}frames, steps={steps}")
...
wf = {
    ...
    "71": {"class_type": "SaveVideo", "inputs": {"video": ["70",0], "filename_prefix": "test_lora", "format": "mp4", "codec": "h264"}},
}
            ...
            for no, out in h[pid].get("outputs",{}).items():
                for v in out.get("images", out.get("media",[])):
                    ...
                    if rv and os.path.exists(rv) and rv.endswith('.mp4'):
                        ...
                        subprocess.run(["ffmpeg","-y","-i",rv,"-i",vocals_8s,
