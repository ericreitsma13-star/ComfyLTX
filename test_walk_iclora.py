#!/usr/bin/env python3
"""Standard flow: IC-LoRA (clean walking ref) + audio conditioning. No trained LoRA."""
...
PROMPT = "young woman with dark brown hair walking through misty pine forest at dawn, golden light, cinematic, full body shot, natural movement"
NEGATIVE = "ugly, deformed, bad anatomy, blurry, music, instruments, close up"
W, H, FPS = 960, 576, 24
DURATION = 6.0
SEED = 42
steps, cfg = 25, 3.0
...
wf = {
    ...
    "53": {"class_type": "CFGGuider", "inputs": {"model": ["20", 0], "positive": ["47", 0], "negative": ["47", 1], "cfg": float(cfg)}},
    ...
    "71": {"class_type": "SaveVideo", "inputs": {"video": ["70", 0], "filename_prefix": "walk_iclora", "format": "mp4", "codec": "h264"}},
}

print(f"Walking IC-LoRA: {DURATION}s ({frame_count}fr) @ {W}x{H}, ref=walking pose")
...
start = time.time(); raw_video = None
...
if raw_video and os.path.exists(raw_video):
    final = raw_video.replace(".mp4","_final.mp4")
    subprocess.run(["ffmpeg","-y","-i",raw_video,"-i","/home/ericr/ComfyUI/input/pines_original_6s.mp3",
        "-c:v","copy","-c:a","aac","-b:a","192k","-map","0:v:0","-map","1:a:0","-shortest","-movflags","+faststart",final],
        check=True, capture_output=True)
    r = subprocess.run(["ffprobe","-v","error","-show_entries","format=duration,size","-of","csv=p=0",final],
        capture_output=True, text=True)
    print(f"Final: {final} ({r.stdout.strip()})")
